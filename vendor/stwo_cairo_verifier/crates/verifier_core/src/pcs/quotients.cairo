...
const SAMPLE_POINT_TAG_PERIODICITY: u32 = 0;
const SAMPLE_POINT_TAG_PREV: u32 = 1;
const SAMPLE_POINT_TAG_OODS: u32 = 2;
const SAMPLE_POINT_TAG_NEXT: u32 = 3;

type SamplesWithRandomness = Span<Span<Span<(u32, QM31, QM31)>>>;
...
pub fn fri_answers(
    ...
    let samples_with_randomness: SamplesWithRandomness = build_samples_with_randomness(
    ...
    // * the quotient constants corresponding to the sample batches for log degree bound i.
...
pub(crate) fn sample_batches_for_degree_bound(
    column_indices_per_tree: @TreeSpan<Span<usize>>,
    sample_values_with_rand: SamplesWithRandomness,
    oods_point: CirclePoint<QM31>,
    prev_oods_point: CirclePoint<QM31>,
    periodicity_generator: CirclePoint<M31>,
    trace_step: CirclePoint<M31>,
    ...
    /// The triples (column index, evaluation, random coefficient) at the point `Z+g`.
    let mut col_eval_coeff_triples_at_next_point = array![];

    let periodicity_point = oods_point.add_circle_point_m31(periodicity_generator);
    let next_oods_point = oods_point.add_circle_point_m31(trace_step);
    let mut n_point_batches: u32 = 0;
    let mut periodicity_order: u32 = 4;
    let mut prev_order: u32 = 4;
    let mut oods_order: u32 = 4;
    let mut next_order: u32 = 4;
    let mut saw_periodicity_point = false;
    let mut saw_prev_point = false;
    let mut saw_oods_point = false;
    let mut saw_next_point = false;
    ...
    for (column_indices, samples_per_column) in zip_eq(
        *column_indices_per_tree, sample_values_with_rand,
        ...
        for column_idx in column_indices {
            ...
            for sample_data in sample_values_at_column {
                let (point_tag, sample_value, random_pow) = *sample_data;
                if point_tag == SAMPLE_POINT_TAG_PERIODICITY {
                    if !saw_periodicity_point {
                        periodicity_order = n_point_batches;
                        n_point_batches += 1;
                        saw_periodicity_point = true;
                    }
                    col_eval_coeff_triples_at_point_plus_periodicity
                        .append((index, sample_value, random_pow));
                } else if point_tag == SAMPLE_POINT_TAG_PREV {
                    if !saw_prev_point {
                        prev_order = n_point_batches;
                        n_point_batches += 1;
                        saw_prev_point = true;
                    }
                    col_eval_coeff_triples_at_prev_point.append((index, sample_value, random_pow));
                } else if point_tag == SAMPLE_POINT_TAG_OODS {
                    if !saw_oods_point {
                        oods_order = n_point_batches;
                        n_point_batches += 1;
                        saw_oods_point = true;
                    }
                    col_eval_coeff_triples_at_point.append((index, sample_value, random_pow));
                } else {
                    assert!(point_tag == SAMPLE_POINT_TAG_NEXT, "Unexpected sample point tag");
                    if !saw_next_point {
                        next_order = n_point_batches;
                        n_point_batches += 1;
                        saw_next_point = true;
                    }
                    col_eval_coeff_triples_at_next_point.append((index, sample_value, random_pow));
                }
            }
            index += 1;
        }
    ...
    let periodicity_samples = col_eval_coeff_triples_at_point_plus_periodicity.span();
    let prev_samples = col_eval_coeff_triples_at_prev_point.span();
    let oods_samples = col_eval_coeff_triples_at_point.span();
    let next_samples = col_eval_coeff_triples_at_next_point.span();
    let mut sample_batches_by_point: Array<ColumnSampleBatch> = array![];
    let mut point_batch_index: u32 = 0;
    while point_batch_index < n_point_batches {
        if periodicity_order == point_batch_index {
            let mut batch_samples = array![];
            batch_samples.append_span(periodicity_samples);
            sample_batches_by_point
                .append(
                    ColumnSampleBatch {
                        point: periodicity_point,
                        cols_vals_and_pows: batch_samples,
                    },
                );
        } else if prev_order == point_batch_index {
            let mut batch_samples = array![];
            batch_samples.append_span(prev_samples);
            sample_batches_by_point
                .append(
                    ColumnSampleBatch {
        ...
        } else if oods_order == point_batch_index {
            let mut batch_samples = array![];
            batch_samples.append_span(oods_samples);
            sample_batches_by_point
                .append(
                    ColumnSampleBatch {
                        point: oods_point, cols_vals_and_pows: batch_samples,
                    },
                );
        } else {
            assert!(next_order == point_batch_index, "Unexpected sample point tag");
            let mut batch_samples = array![];
            batch_samples.append_span(next_samples);
            sample_batches_by_point
                .append(
                    ColumnSampleBatch {
                        point: next_oods_point, cols_vals_and_pows: batch_samples,
                    },
                );
        }
        point_batch_index += 1;
...
fn build_samples_with_randomness(
    ...
    for sample_values_per_column in sample_values_per_column_per_tree {
        ...
        for sample_values in sample_values_per_column {
            ...
            // Authenticated proofs can already carry the periodicity sample explicitly, in which
            // case we preserve the full triple in-order and only attach the random coefficient.
            // Phil's authentic `proofInputHash` slice proof uses explicit three-point transition
            // masks with point order `prev -> oods -> next`.
            if let Some(triple_box) = (*sample_values).try_into() {
                let [prev_sample, ood_sample, next_sample]: [QM31; 3] = (*triple_box).unbox();
                new_samples.append((SAMPLE_POINT_TAG_PREV, prev_sample, random_pow));
                random_pow *= random_coeff;

                new_samples.append((SAMPLE_POINT_TAG_OODS, ood_sample, random_pow));
                random_pow *= random_coeff;

                new_samples.append((SAMPLE_POINT_TAG_NEXT, next_sample, random_pow));
                random_pow *= random_coeff;
            } else if let Some(tuple_box) = (*sample_values).try_into() {
                let [prev_sample, ood_sample]: [QM31; 2] = (*tuple_box).unbox();
                // Add periodicity sample.
                new_samples.append((SAMPLE_POINT_TAG_PERIODICITY, ood_sample, random_pow));
                random_pow *= random_coeff;
