...
fn reduce_blake2s_words(words: Box<[u32; 8]>) -> Box<[u32; 8]> {
    let [w0, w1, w2, w3, w4, w5, w6, w7] = words.unbox();
    BoxImpl::new(
        [
            M31Trait::reduce_u32(w0).into(),
            M31Trait::reduce_u32(w1).into(),
            M31Trait::reduce_u32(w2).into(),
            M31Trait::reduce_u32(w3).into(),
            M31Trait::reduce_u32(w4).into(),
            M31Trait::reduce_u32(w5).into(),
            M31Trait::reduce_u32(w6).into(),
            M31Trait::reduce_u32(w7).into(),
        ],
    )
}

fn reduce_blake2s_hash(hash: Blake2sHash) -> Blake2sHash {
    Blake2sHash { hash: reduce_blake2s_words(hash.hash) }
}
...
pub impl Blake2sChannelImpl of ChannelTrait {
    fn mix_commitment(ref self: Blake2sChannel, commitment: Blake2sHash) {
        ...
        update_digest(ref self, reduce_blake2s_hash(Blake2sHash { hash: res }));
    }
    ...
    fn verify_pow_nonce(self: @Blake2sChannel, n_bits: u32, nonce: u64) -> bool {
        ...
        let [q0, q1, q2, q3, q4, q5, q6, q7] = reduce_blake2s_words(
            blake2s_finalize(BoxImpl::new(BLAKE2S_256_INITIAL_STATE), 52, msg),
        )
            .unbox();
        ...
        let digest = reduce_blake2s_hash(
            Blake2sHash {
